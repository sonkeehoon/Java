...
배운지 몇년 지나서 Java에 대한 감이 떨어지는 시점에 다시 복습해보려고 한다
...
## `2023-09-30` Java 12일차

1. 나의 앱 만들기 (후반부)
- 1. 메소드 : 서로 연관된 코드를 그룹핑해서 이름을 붙인 정리정돈의 상자
- 2. 클래스 : 클래스는 서로 연관된 변수와 메소드를 그룹핑해서 이름을 붙인 것 (정리정돈의 상자)
- 3. 인스턴스 : 하나의 클래스를 복제해서 서로 다른 데이터의 값과 서로 같은 메소드를 가진 복제본을 만드는 것
